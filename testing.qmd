...
# assoc rules
rules <- apriori(Groceries, parameter = list(support = 0.01, confidence = 0.5, minlen = 1))
apriori_rules <- inspect(rules)


itemsets <- eclat(Groceries, parameter = list(supp = 0.01))
rules <- ruleInduction(itemsets, confidence = .5)
eclat_rules <- inspect(rules)


left_join(apriori_rules, eclat_rules, by = c('lhs', 'rhs'))


# itemsets
apriori_itemsets <- apriori(Groceries, parameter = list(support = 0.01, confidence = 0.5, minlen = 1, target = 'frequent itemsets'))
apriori_itemsets <- inspect(apriori_itemsets)

left_join(apriori_itemsets, inspect(itemsets), by = c('items, support'))
