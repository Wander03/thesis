...
The following example uses the `palmerPenguins` dataset [@horst_palmerpenguins_2020] to create a recipe for k-means clustering:
...
Since unsupervised learning algorithms have no target or outcome variables, there is no inherent metric for measuring the success of predictions. However, some metrics are implemented in `tidyclust` to measure the quality of the created clusters. For example, when evaluating results of k-means clustering, researchers often use two metrics: SSE and the silhouette score. Here we omit the details of the metrics and show only the structure of the code to calculate them from a fitted cluster model:
...
Having established the background of FIM and `tidyclust`, this chapter presents the core solution: the implementation of a `freq_itemsets` model in `tidyclust`.
...
The `predict()` function for `freq_itemsets` models introduces a novel methodology for generating predictions, which is an original contribution of this thesis. Unlike the conventional use of frequent itemsets, which typically focuses on generating association rules, this implementation frames the task as a recommender system. The following algorithm predicts missing items in a new transaction based on the frequent itemsets learning from the training data. For example, suppose a customer in a grocery store has milk and bread in their cart. This system would predict which other items they are likely to add based on historical purchasing patterns where milk and bread appeared together.
...
This work provides new functionality in the `tidyclust` package by implementing a cluster specification for frequent itemset mining. To handle the unique output of the Apriori and Eclat algorithms, substantial modifications were made to the existing `fit()` and `predict()` methods. Furthermore, several new functions were built to support this implementation. These include `extract_predictions()` which reformats the complex prediction output to a user-friendly format, and `augment_itemset_predict()` that prepares prediction output for evaluation using standard metric functions.

Additionally, this work serves as a foundation that can be built upon to incorporate other unsupervised and semi-supervised algorithms into `tidyclust` that provide column-clustering. Examples of algorithms that may follow this new framework include latent dirichlet allocation, matrix factorization and recommender systems, or factor analysis.
...
It is important to note that the `tidyclust` package acts as a wrapper and does 
not add computational overhead; performance is determined by the underlying engine
package, in this case `arules`. Furthermore, the prediction method was designed
to be lightweight, ensuring that generating predictions is not a computationally
intensive process.
  ...
  while (changed && iter < max_iter) {
    ...
    for (i in 1:length(items)) {
      ...
      relevant_itemsets <- which(
        sapply(itemset_list, function(x) current_item %in% x)
        )
