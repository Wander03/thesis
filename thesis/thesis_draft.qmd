...
Since unsupervised learning algorithms have no target or outcome variables, there is no inherent metric for measuring the success of predictions. However, some metrics are implemented in `tidyclust` to measure the quality of the created clusters. For partition clustering like k-means, these metrics are sum of squared error (SSE) and silhouette. Here we omit the details of the metrics and show only the structure of the code to calculate them from a fitted cluster model:
...
Additionally, this work serves as a foundation that can be built upon to incorporate other semi-supervised algorithms into `tidyclust`. The implemented structure provides a clear path for integrating new item-based clustering techniques and for introducing a formal `association_rules()` model, further centralizing data mining tasks within the `tidyclust` framework.
