...
The name of the function, `k_means`, is the name of the model. The parameter(s), here `num_clusters`, are the required and optional inputs for your model. In this example `num_clusters = 3` represents a K-means model with three clusters. The `set_engine()` function determines what library implementation of the model is being run, and in this example that is the `stats` library. 

The next step is data preparation, and is taken care of by `recipes` [@kuhn_recipes_2024]. The `recipes` library support a range of data transformations including normalization, standardization, and dummy variable creation as well as data cleaning steps such as missing value removal and imputation. The specification of a recipe follows a declarative syntax, improving both readability and reproducibility in the data cleaning and preparation process.

```{r}
recipe <- recipe(~ bill_length_mm + bill_depth_mm + sex, data = penguins) %>%
  step_naomit(all_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors())
```

The recipe example above is creating a formula using the `PalmerPenguins` dataset, where `bill_length_mm`, `bill_depth_mm`, and `sex` are the predictor variables. It removes missing values, normalizes the numeric predictors, and converts the categorical predictor into dummy variables. This recipe can be used for different models
